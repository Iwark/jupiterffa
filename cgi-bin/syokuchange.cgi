...
&decode;
	...
	$back_form = << "EOM";
<br>
<form action="syokuchange.cgi" method="post">
...
EOM
if($mon==1 and $mday==1){
	$tensyokuryoukin = $chara[37] * 500000 / 2;
	print "<font color=\"red\" size=5>お正月イベント！転職費用が半額の$tensyokuryoukinＧになります！！</font>";
}
	print <<"EOM";
...
EOM
	...
	$syohit=0;
	for($i=0;$i<51;$i++){
		if($syoku_master[$i]==100){$syohit++;}
	}
	if($syohit>=50){$syocom="あなたは、『100回転生』で、100回分まとめて転生が可\能\です。費用は半額になります。";}
	&header;
...
EOM
if($mon==1 and $mday==1){
	$tenseiryoukin = 1000000 * $chara[37] / 2;
	print "<font color=\"red\" size=5>お正月イベント！転生費用が半額の$tenseiryoukinＧになります！！</font>";
}
if($chara[70]<1){
	print <<"EOM";
$tensyokuok</tr></table><br>
...
<input type=hidden name=id value="$chara[0]">
<input type="hidden" name="mydata" value="$chara_log">
<input type=hidden name=mode value="tensei_change">
EOM
...
<input type=hidden name=id value="$chara[0]">
<input type="hidden" name="mydata" value="$chara_log">
<input type=hidden name=mode value="tensei_100change">
<input type=\"submit\" class=btn value=\"100回転生する\">
</form>
EOM
}
	print <<"EOM";
...
sub tensyoku_change {
	...
	$tensyokuryoukin = $chara[37] * 500000;
	if($mon==1 and $mday==1){$tensyokuryoukin = $chara[37] * 500000 / 2;}
...
sub tensei_change {
	...
	$tenseiryoukin = 1000000 * $chara[37];
	if($mon==1 and $mday==1){$tenseiryoukin = 1000000 * $chara[37] / 2;}
...
sub tensei_100change {
	...
	$tenseiryoukin = 50000000 * $chara[37];
	if($mon==1 and $mday==1){$tenseiryoukin = 50000000 * $chara[37] / 2;}
	if($chara[33]<$tenseilevel) {&error("レベルが足りません");}
	...
	$boss="$chara[4]様が100回転生で$chara_syoku[$chara[14]]に転生しました";
	$text_color = "#66FF99";
	$text_size = 13;
