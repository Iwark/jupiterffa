...
sub make_pre {
	...
	elsif ( -e "./echaralog/$in{'id'}.cgi") {
		&make_error("そのＩＤのデータは、オートバックアップされています。掲示板から復旧申請してください。");
	}
...
<input type=submit class="btn" value="作成する">
</form>
<form action="$chara_make" >
<input type=hidden name="id" value="$in{'id'}">
<input type=hidden name="pass" value="$in{'pass'}">
...
sub make_end {
	...
	$new_chara = "$in{'id'}<>$in{'pass'}<>$in{'site'}<>$in{'url'}<>$in{'c_name'}<>$in{'sex'}<>$in{'chara'}<>1<>1<>1<>1<>1<>1<>3<>$in{'syoku'}<>$kiso_hp<>$kiso_hp<>0<>1<>$intgold<>1<>0<>0<>$clt_comment<>$first_item<>$sentou_limit<>$host<>$newdata<>$boss<>$first_def<>0<>$first_acs<>0<>1<>";
	...
	$boa="<font color=\"blue\">$in{'c_name'}様が新たにキャラ作成されました！みなさんよろしく！</font>";
...
<tr>
<td rowspan="7" align="center"><img src="$img_path/$chara_img[$in{'chara'}]"></td>
<td class="b1">なまえ</td>
<td>$in{'c_name'}</td>
<td class="b1">性別</td>
<td>$esex</td>
