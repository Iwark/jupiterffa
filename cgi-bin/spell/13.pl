sub spell13{
	...
	if(int(rand($rand))==0){
		$hpplus1 = int(${'dmg'.$sp}/10);
		$hpplus2 = int(${'dmg'.$sp}/10);
		$hpplus3 = int(${'dmg'.$sp}/10);
		$hpplus4 = int(${'dmg'.$sp}/10);
		${'dmg'.$sp} = 0;
		if($sp==1){
			${'com'.$sp} .="<font class=\"white\" size=5>$chara[4]は白魔法ヒールガを放った!!</font><br>";
			...
			$ri=$sp-1;
			${'com'.$sp} .="<font class=\"white\" size=5>${'mem'.$ri}[4]は白魔法ヒールガを放った!!</font><br>";
	...
	if($mahoken==1){$rand=4;}else{$rand=1;}
	if(int(rand($rand))==0){
		$shpplus1 = int(${'sdmg'.$ssp}/10);
		$shpplus2 = int(${'sdmg'.$ssp}/10);
		$shpplus3 = int(${'sdmg'.$ssp}/10);
		$shpplus4 = int(${'sdmg'.$ssp}/10);
		${'sdmg'.$ssp} = 0;
		${'scom'.$ssp} .="<font class=\"white\" size=5>${'smem'.$ssp}[4]は白魔法ヒールガを放った!!</font><br>";
