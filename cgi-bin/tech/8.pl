		...
		if($abi==$ab){
			if($ab==1){
				if($waza_ritu1 + int(rand($chara[12]/4)) > int(rand(500))){
					${'com'.$ab} .="<font class=\"red\" size=5>必殺技ぶんどる！！！</font><br>";
					$k=1;
					$drop_plus += 1;
					$ahit=0;
					if($place==0){$pp=1;}
					...
					if($ahit!=1){
						...
						if($ssmname1 eq "エッグ" and int(rand(15))==0 and $chara[18]>60){
							if($chara[38]<3001){
					$com1 .= "<b><font size=4 color=red>エッグをぶんどった！！</font></b><br>";
							$php_flg = 300;
							$chara[38] = 3003;
							$chara[39] = "エッグ";
							$chara[40] = 0;
				...
				if($i_name){$com1 .="<font class=\"yellow\" size=5>$i_nameをぶんどった！！<br>";}
				$i_name="";
				}
			}else{
				$ri=$ab-1;
				if(${'waza_ritu'.$ab} + int(rand(${'mem'.$ri}[12] / 4)) > int(rand(500))){
					$k=1;
					${'com'.$ab} .="<font class=\"red\" size=5>必殺技ぶんどる！！！</font><br>";
					${'com'.$ab} .="<font class=\"red\" size=5>何かぶんどれた？？</font><br>";
				}
			}
		}
	}
	for($abi=1;$abi<5;$abi++){
		if($abi==$sab){
			if(${'swaza_ritu'.$sab} + int(rand(${'smem'.$sab}[12] / 4)) > int(rand(500))){
				$k=1;
				${'scom'.$sab} .="<font class=\"red\" size=5>必殺技ぶんどる！！！</font><br>";
				${'com'.$ab} .="<font class=\"red\" size=5>何かぶんどれた？？</font><br>";
			}
		}
	}
